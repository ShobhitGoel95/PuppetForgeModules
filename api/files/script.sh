...
#modify by Devops
#path="/etc/puppetlabs/code/environments/production/modules/api/files"
#path_API=$path/APIGateway_7.7_Install_linux-x86-64_BN4.run
source /etc/puppetlabs/code/environments/production/modules/installationDetails.txt
cd /etc/puppetlabs/code/environments/production/modules/files
...
./APIGateway_7.7_Install_linux-x86-64_BN4.run --mode unattended --setup_type advanced --licenseFilePath $licenseFilePath --prefix $dir --enable-components $enable_components --disable-components $disable_components --username $username --adminpasswd $adminpasswd --cassandraInstalldir $cassandraInstalldir --cassandraJDK $cassandraJDK --startCassandra $cassandraStart --analyticsLicenseFilePath $analyticsLicenseFilePath --apimgmtLicenseFilePath $apimgmtlicenseFilePath

if [ ! -d "/Axway/API-7.7/apigateway/conf/licenses" ]
then
  echo "Inside if loop------->>>>>>>>> creating dir"
  sudo mkdir -p /Axway/API-7.7/apigateway/conf/licenses/
  sudo chmod -R 775 /Axway/API-7.7/apigateway/conf/licenses/
  sudo cp /etc/puppetlabs/code/environments/production/modules/files/license.lic /Axway/API-7.7/apigateway/conf/licenses/
fi
